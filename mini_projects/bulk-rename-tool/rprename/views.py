...
class Window(QWidget, Ui_Window):
    def __init__(self):
        super().__init__()
        # Store paths to the files to be renamed
        self._files = deque()
    ...
    # Trigger .loadFiles() every time user clicks on Load Files button
    ...
    def loadFiles(self):
        # Clear .dstFileList list widget every time user clicks Load Files
        ...
        # Returns a list of string-based paths to the selected files with the currently used file filter
        files, filter = QFileDialog.getOpenFileNames(
            self, "Choose Files to Rename", initDir, filter=FILTERS)
        if len(files) > 0:
            # Slices current filter string to extract the file extension
            fileExtension = filter[filter.index("*") : -1]
            self.extensionLabel.setText(fileExtension)
            # Get directory path to the selected files
            srcDirName = str(Path(files[0]).parent)
            self.dirEdit.setText(srcDirName)
            for file in files:
                self._files.append(Path(file))
                self.srcFileList.addItem(file)
            # Update the current number of files in the list
            self._filesCount = len(self._files)
